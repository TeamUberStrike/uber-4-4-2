...
public class InventoryManager : Singleton<InventoryManager>
{
	...
	public IEnumerator StartUpdateInventoryAndEquipNewItem(IUnityItem item, bool equipNow = false)
	{
		if (item != null)
		{
			...
			{
				Singleton<HudPopup>.Instance.Show("Your purchase was successful!\nIt will be available next life!", item);
				EquipItem(item);
			}
	...
	public bool EquipItem(IUnityItem item)
	{
		...
		InventoryItem item2;
		if (TryGetInventoryItem(item.View.ID, out item2) && item2.IsValid && item2.Item.View.ItemType == UberstrikeItemType.Weapon && GameState.CurrentGame != null)
		{
			slotType = RecommendationUtils.FindBestSlotToEquipWeapon(item2.Item, RecommendationUtils.GetCategoriesForCombatRange(GameState.CurrentSpace.CombatRangeTiers));
		}
	...
	public void UnequipWeaponSlot(LoadoutSlotType slotType)
	{
		...
		GameState.LocalAvatar.Decorator.SetActiveWeaponSlot(slotType);
		GameState.LocalAvatar.Decorator.UnassignWeapon(slotType);
	}
	...
	public bool EquipItemOnSlot(int itemId, LoadoutSlotType slotType)
	{
		...
		if (TryGetInventoryItem(itemId, out item) && item.IsValid)
		{
			if (Singleton<LoadoutManager>.Instance.IsItemEquipped(itemId))
			{
				...
				{
					ShopHighlightSlotEvent shopHighlightSlotEvent = new ShopHighlightSlotEvent();
					shopHighlightSlotEvent.SlotType = slot;
					CmuneEventHandler.Route(shopHighlightSlotEvent);
			...
			else
			{
				...
				Singleton<TemporaryLoadoutManager>.Instance.SetGearLoadout(slotType, item.Item);
				...
				bool resetAnimations = item.Item.View.ItemClass == UberstrikeItemClass.GearHolo;
				AutoMonoBehaviour<AvatarAnimationManager>.Instance.SetAnimationState(PageType.Shop, item.Item.View.ItemClass, resetAnimations);
				ShopHighlightSlotEvent shopHighlightSlotEvent = new ShopHighlightSlotEvent();
				shopHighlightSlotEvent.SlotType = slotType;
				CmuneEventHandler.Route(shopHighlightSlotEvent);
			}
			return true;
		}
