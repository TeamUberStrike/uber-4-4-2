...
public class SilhouetteOutlined : ImageEffectBase
{
	...
	private void OnPreRender()
	{
		...
		{
			CleanRenderTextures();
			Camera camera = ShaderCamera.camera;
			camera.CopyFrom(base.camera);
			camera.backgroundColor = new Color(0f, 0f, 0f, 0f);
			camera.clearFlags = CameraClearFlags.Color;
			_maskTexture = RenderTexture.GetTemporary((int)base.camera.pixelWidth, (int)base.camera.pixelHeight, 16);
			camera.targetTexture = _maskTexture;
			camera.RenderWithShader(objectMaskShader, "Outline");
			UpdateGlowTextureSize(base.camera.pixelWidth, base.camera.pixelHeight);
