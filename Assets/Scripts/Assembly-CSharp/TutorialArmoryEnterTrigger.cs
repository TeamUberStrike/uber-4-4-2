...
public class TutorialArmoryEnterTrigger : MonoBehaviour
{
	...
	private IEnumerator StartEnterArmory()
	{
		...
		while (Vector3.Dot(LevelCamera.Instance.TransformCache.position - LevelTutorial.Instance.ArmoryCameraPathEnd.position, Vector3.right) > 0f && Vector3.Distance(LevelCamera.Instance.TransformCache.position, LevelTutorial.Instance.ArmoryCameraPathEnd.position) > 0.1f)
		{
			...
			{
				played = true;
				LevelTutorial.Instance.NPC.GetComponent<Animation>().Blend(AnimationIndex.TutorialGuideIdle.ToString(), 0f);
				LevelTutorial.Instance.NPC.GetComponent<Animation>().Blend(AnimationIndex.TutorialGuideArmory.ToString(), 1f);
			}
		}
		...
		LevelTutorial.Instance.NPC.GetComponent<Animation>().Blend(AnimationIndex.TutorialGuideArmory.ToString(), 0f);
		LevelTutorial.Instance.NPC.GetComponent<Animation>().Blend(AnimationIndex.TutorialGuideTalk.ToString(), 1f);
		yield return new WaitForSeconds(LevelTutorial.Instance.VoicePickupWeapon.length);
		LevelTutorial.Instance.NPC.GetComponent<Animation>().Blend(AnimationIndex.TutorialGuideTalk.ToString(), 0f);
		LevelTutorial.Instance.NPC.GetComponent<Animation>().Blend(AnimationIndex.TutorialGuideIdle.ToString(), 1f);
