...
public class ReportPlayerPanelGUI : PanelGuiBase
{
	...
	public static void ReportInboxPlayer(PrivateMessageView msg, string messageSender)
	{
		...
		if (CommConnectionManager.IsConnected)
		{
			...
			{
				CommConnectionManager.CommCenter.SendPlayerReport(new int[1] { reportedCmid }, MemberReportType.OffensiveChat, reason);
	...
	private void OnMemberListUpdatedEvent(MemberListUpdatedEvent ev)
	{
		...
		int num = 0;
		ActorInfo[] players = ev.Players;
		foreach (ActorInfo actorInfo in players)
		{
			_playerNameTexts[num++] = actorInfo.PlayerName;
		}
	}
}
