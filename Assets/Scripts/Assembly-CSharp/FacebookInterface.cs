...
public class FacebookInterface : AutoMonoBehaviour<FacebookInterface>
{
	...
	private static extern void usHelperPublishScore(string score);
#endif
	...
	public void PublishScore(int score)
	{
		...
		{
#if UNITY_WEBGL && !UNITY_EDITOR
			usHelperPublishScore(score.ToString());
#endif
		}
	}
	...
	private IEnumerator GetScreenshot()
	{
		...
		{
			screenshotCam.GetComponent<Camera>().CopyFrom(GameState.CurrentSpace.Camera);
			screenshotCam.GetComponent<Camera>().targetTexture = tx;
			screenshotCam.GetComponent<Camera>().rect = new Rect(0f, 0f, 1f, 1f);
			screenshotCam.GetComponent<Camera>().Render();
		...
		{
			screenshotCam.GetComponent<Camera>().CopyFrom(GameState.LocalPlayer.WeaponCamera.GetComponent<Camera>());
			screenshotCam.GetComponent<Camera>().targetTexture = tx;
			screenshotCam.GetComponent<Camera>().rect = new Rect(0f, 0f, 1f, 1f);
			screenshotCam.GetComponent<Camera>().Render();
			wasScreenshotTaken = true;
		...
		if (wasScreenshotTaken)
		{
			...
			UnityWebRequest w = UnityWebRequest.Post("https://graph.facebook.com/me/photos?access_token=" + FB.AccessToken, form);
		...
		UnityEngine.Object.Destroy(screenshotCam);
	}
