...
public class TabScreenPanelGUI : MonoBehaviour
{
	...
	private string MapName
	{
		get
		{
			return LocalizedStrings.Map + ": " + Singleton<MapManager>.Instance.GetMapName(GameState.CurrentSpace.SceneName);
		}
	}
	...
	private void Awake()
	{
		...
		_panelSize.x = 700f;
		_panelSize.y = 400f;
		_allPlayers = new List<UberStrike.Realtime.UnitySdk.CharacterInfo>(0);
		_redTeam = new List<UberStrike.Realtime.UnitySdk.CharacterInfo>(0);
		_blueTeam = new List<UberStrike.Realtime.UnitySdk.CharacterInfo>(0);
	}
	...
	private void OnGUI()
	{
		...
		bool flag = (AutoMonoBehaviour<InputManager>.Instance.RawValue(GameInputKey.Tabscreen) > 0f || ForceShow) && !PanelManager.IsAnyPanelOpen && GameState.CurrentGame.CanShowTabscreen;
		if (Enabled != flag)
		{
			Enabled = flag;
			if (flag && SortPlayersByRank != null)
			{
	...
	private Vector2 DoTeam(Rect position, TeamID teamID, Vector2 scroll, List<UberStrike.Realtime.UnitySdk.CharacterInfo> players)
	{
		...
		if (teamID == TeamID.RED)
		{
			...
			GUI.Label(new Rect(position.width - 215f, 8f, 200f, 48f), _redTeamScore.ToString(), BlueStonez.label_interparkbold_48pt_right);
		}
		else
		{
			GUI.Label(new Rect(15f, 8f, 200f, 48f), _blueTeamScore.ToString(), BlueStonez.label_interparkbold_48pt_left);
	...
	public void SetPlayerListAll(List<UberStrike.Realtime.UnitySdk.CharacterInfo> players)
	{
		_allPlayers = players;
	}
	...
	private Vector2 DoAllStats(Rect position, Vector2 scroll, List<UberStrike.Realtime.UnitySdk.CharacterInfo> players)
	{
		...
		foreach (UberStrike.Realtime.UnitySdk.CharacterInfo player in players)
		{
			...
			GUI.Label(new Rect(num13, 0f, num12, 36f), player.PlayerName, BlueStonez.label_interparkbold_11pt_left_wrap);
			GUI.color = contentColor;
			num13 += num12;
			...
			if (num8 > 0)
			{
				GUI.Label(new Rect(num13, 0f, num8, 36f), player.KDR.ToString("N1"), BlueStonez.label_interparkbold_11pt_left);
				num13 += num8;
			}
			if (!player.IsAlive)
			{
				GUI.Label(new Rect(num13, 6f, 25f, 25f), _iconSplatted, BlueStonez.label_interparkbold_11pt_right);
			}
			num13 += num10;
	...
	public void SetPlayerListRed(List<UberStrike.Realtime.UnitySdk.CharacterInfo> redPlayers)
	{
		_redTeam = redPlayers;
	}

	public void SetPlayerListBlue(List<UberStrike.Realtime.UnitySdk.CharacterInfo> bluePlayers)
	{
