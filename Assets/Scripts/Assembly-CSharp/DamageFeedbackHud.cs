	...
	public void Draw()
	{
		if (Enabled)
		{
			for (int i = 0; i < _damageFeedbackMarkList.Count; i++)
			{
				...
				Vector3 direction = Quaternion.Euler(0f, damageDirection, 0f) * Vector3.back;
				direction = LevelCamera.Instance.TransformCache.InverseTransformDirection(direction);
