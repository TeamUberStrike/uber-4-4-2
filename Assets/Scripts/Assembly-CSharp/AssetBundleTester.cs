...
public class AssetBundleTester : MonoBehaviour
{
	...
	{
		_isReadingUrl = true;
		UnityWebRequest www = UnityWebRequest.Get(baseUrl + "Items/4.4.2/ItemAssetBundle.xml?arg=" + DateTime.Now.Ticks);
		yield return www.SendWebRequest();
		if (www.result == UnityWebRequest.Result.Success)
		{
			_bundlePath = _baseUrl + "Items/4.4.2/" + GetItemAssetBundleName(www.downloadHandler.text);
		}
		_isReadingUrl = false;
	}
	...
	private IEnumerator InstantiateItems(AssetBundle bundle)
	{
		...
		if ((bool)bundle)
		{
			UnityEngine.Object[] array = bundle.LoadAllAssets(typeof(GameObject));
