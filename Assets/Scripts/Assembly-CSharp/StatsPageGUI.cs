...
public class StatsPageGUI : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		CmuneEventHandler.AddListener<LogoutEvent>(delegate
		{
			_selectedStatsTab = 0;
	...
	{
		XpPointsUtil.GetXpRangeForLevel(PlayerDataManager.PlayerLevelSecure, out _playerCurrentLevelXpReq, out _playerNextLevelXpReq);
	...
	private void DrawXPMeter(Rect position)
	{
		...
		{
			GUI.Label(new Rect(0f, 0f, 200f, 16f), LocalizedStrings.CurrentXP + " " + PlayerDataManager.PlayerExperience, BlueStonez.label_interparkbold_11pt_left);
			GUI.Label(new Rect(position.width - 200f, 0f, 200f, 16f), LocalizedStrings.RemainingXP + " " + Mathf.Max(0, _playerNextLevelXpReq - PlayerDataManager.PlayerExperience), BlueStonez.label_interparkbold_11pt_right);
