...
public class Sprite2D : IAnimatable2D
{
	...
	public Vector2 Size
	{
		get
		{
			_size.x = Scale.x * (float)_texture.width;
			_size.y = Scale.y * (float)_texture.height;
			return _size;
		}
	}
	...
	private void UpdateRect()
	{
		if ((bool)_texture)
		{
			...
			_rect.y = Position.y;
			_rect.width = (float)_texture.width * Scale.x;
			_rect.height = (float)_texture.height * Scale.y;
			return;
		}
