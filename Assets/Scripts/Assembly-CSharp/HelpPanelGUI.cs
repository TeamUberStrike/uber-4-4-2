...
public class HelpPanelGUI : PanelGuiBase
{
	...
	private void DrawGameplayGroup()
	{
		...
		yOffset = DrawGroupLabel(yOffset, "Earning XP", "There are five XP granting events in UberStrike. XP events stack, so if for example you get a headshot splat with a melee weapon you get 3 XP, one for the splat, one for the headshot, and one for the melee.");
		yOffset = DrawGroupLabel(yOffset, "Splatting an Enemy", "When you deal the final below to an enemy you get 1 XP.");
		yOffset = DrawGroupLabel(yOffset, "Headshot Splats", "When you splat an enemy with a headshot you get 1 XP.");
		yOffset = DrawGroupLabel(yOffset, "Nutshot Splats", "When you splat an enemy with a nutshot you get 1 XP.");
		yOffset = DrawGroupLabel(yOffset, "Melee Splats", "When you splat an enemy with a melee weapon you get 1 XP.");
		yOffset = DrawGroupLabel(yOffset, "Deal 100 Damage", "For every 100 damage you deal you get 1 XP.");
		...
		yOffset = DrawGroupLabel(yOffset, "Armor Points", "Armor Points are picked up in the game. They absorb a percentage of the damage you receive. The percentage depends on the gear you have equipped.");
		...
		yOffset = DrawGroupLabel(yOffset, "Selecting Different Weapons", "By scrolling the mouse wheel you can cycle through all of your available weapons. You can also choose specific weapons by pressing the number keys 1 through 5.");
	...
	private void DrawItemsGroup()
	{
		...
		GUI.BeginGroup(rect);
		float yOffset = 10f;
		yOffset = DrawGroupLabel(yOffset, "Weapons", "Your character gets access to weapons after you buy them. You can also pick them up within the game, but you do not get to keep them when you are splatted or when you leave the game. Weapons are divided into eight classes: Melee, Handguns, Machine Guns, Shotguns, Sniper Rifles, Splatter Guns, Cannons, and Launchers. Each weapon class functions differently in game and is applicable in different combat contexts. For example, shotgun class weapons are generally better for close range battles, while the sniper rifle class weapons are better from a distance. Weapons have an expiry date of up to 90 days, depending on how long you buy them for. After the expiry date has passed, the weapon will disappear from your inventory.");
		yOffset = DrawGroupLabel(yOffset, "Gear", "Gear items are used to customize your character and increase your in-game protection. They have an effect on your speed, the amount of AP you can carry without depletion, and the amount of damage each AP absorbs.");
		yOffset = DrawGroupLabel(yOffset, "Loadout", "The Loadout is a list of all items that you own that your character currently has equipped. Your loadout dictates your character抯 appearance in the game.");
		yOffset = DrawGroupLabel(yOffset, "Inventory", "The Inventory is a list of all the items that you own that your character does NOT have equipped. All items here have an expiry time, after which they will disappear from your inventory.");
		...
		yOffset = DrawGroupLabel(yOffset, "Underground", "The Underground is a special shop that sells rare and unique items that cannot be found elsewhere. Items in the Underground have no level restrictions.");
		yOffset = DrawGroupLabel(yOffset, "Points", "Points are used to purchase items from the Shop. You get at least 500 points every day you log into UberStrike.");
		yOffset = DrawGroupLabel(yOffset, "Credits", "Credits are used to purchase powerful items from the Shop. You can obtain credits by clicking on the 'Get Credits' button in the bottom right hand corner of the shop, or on the Taskbar at the top of the screen. Credits are the only currency that can be used to purchase rare items in the Underground.");
		GUI.EndGroup();
		GUITools.EndScrollView();
	}
