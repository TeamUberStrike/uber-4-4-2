...
public class LevelUpPopup : IPopupDialog
{
	...
	public LevelUpPopup(int newLevel, int previousLevel, Action action = null)
	{
		...
		SfxManager.Play2dAudioClip(GameAudio.LevelUp);
	...
	private void DrawPlayGUI(Rect rect)
	{
		...
		{
			num5 = 20;
			if (GUI.Button(new Rect(rect.width * 0.5f - 160f, rect.height - 47f, 140f, 30f), "SHARE", BlueStonez.buttongold_large_price))
			{
				AutoMonoBehaviour<FacebookInterface>.Instance.PublishLevelUp(_level);
			}
		}
