...
public class SkinnedMeshCombiner
{
	...
	private static void CopyAnimation(GameObject target, List<SkinnedMeshRenderer> renderers)
	{
		...
		{
			if (!(renderer.transform != target.transform) || !renderer.GetComponent<Animation>())
			{
				continue;
			}
			Animation animation = renderer.GetComponent<Animation>();
			if (animation.enabled && (bool)animation.clip)
			{
				Animation animation2 = target.GetComponent<Animation>();
				if (!animation2)
				{
