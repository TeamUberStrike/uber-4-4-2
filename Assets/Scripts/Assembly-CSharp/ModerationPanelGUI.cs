...
public class ModerationPanelGUI : PanelGuiBase
{
	...
	private void DrawModerationPanel()
	{
		...
		if (PlayerDataManager.AccessLevel >= MemberAccessLevel.SeniorQA)
		{
			...
			if (GUITools.Button(new Rect(10f, _rect.height - 10f - 32f, 150f, 32f), new GUIContent((!(_nextUpdate < Time.time)) ? string.Format("Next Update ({0:N0})", _nextUpdate - Time.time) : "GET ALL PLAYERS"), BlueStonez.buttondark_medium))
			{
	...
	private void DoPlayerSelection(Rect position)
	{
		...
		foreach (CommUser item in collection2)
		{
			if (string.IsNullOrEmpty(value) || item.Name.ToLower().Contains(value))
			{
				...
				if (GUI.Button(new Rect(10f, num * 20, position.width, 20f), item.Name, BlueStonez.label_interparkmed_10pt_left))
				{
	...
	{
		if (PlayerDataManager.AccessLevelSecure <= MemberAccessLevel.Default || !_moderations.Exists((Moderation m) => m.ID == _moderationSelection))
		{
			return;
		}
		...
		case Actions.UNMUTE_PLAYER:
			...
			{
				ServerRequest.Run(this, _selectedCommUser.CurrentGame.Server, 25, PlayerDataManager.AuthToken, _selectedCommUser.CurrentGame.Number, _selectedCommUser.ActorId, false);
			}
			else
			{
		...
		case Actions.MUTE_PLAYER:
			if (_selectedCommUser.IsInGame)
			{
				ServerRequest.Run(this, _selectedCommUser.CurrentGame.Server, 24, PlayerDataManager.AuthToken, _selectedCommUser.CurrentGame.Number, _selectedCommUser.ActorId, true);
			}
			else
			{
		...
		case Actions.KICK_FROM_GAME:
			if (_selectedCommUser.IsInGame)
			{
				...
				int number = _selectedCommUser.CurrentGame.Number;
				string server = _selectedCommUser.CurrentGame.Server;
				ServerRequest.Run(this, server, 22, PlayerDataManager.AuthToken, cmid, number, num);
