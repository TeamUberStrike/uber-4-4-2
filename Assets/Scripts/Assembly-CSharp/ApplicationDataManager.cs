...
public static class ApplicationDataManager
{
	...
	private static extern void displayMessage(string title, string url);
	
	[DllImport("__Internal")]
	private static extern void getCreditsWrapper(int cmidSecure);
#endif
	...
	public static ChannelType Channel
	{
		get
		{
			if (Application.platform == RuntimePlatform.WebGLPlayer&& WebPlayerSrcValues.HasValues)
			{
	...
	static ApplicationDataManager()
	{
		...
		if (Application.platform == RuntimePlatform.WebGLPlayer)
		{
			...
			WebPlayerSrcValues = new WebPlayerSrcValues(queryString);
		}
	...
	public static void OpenBuyCredits()
	{
		...
		case ChannelType.Kongregate:
			ScreenResolutionManager.IsFullScreen = false;
#if UNITY_WEBGL && !UNITY_EDITOR
			getCreditsWrapper(PlayerDataManager.CmidSecure);
#endif
			break;
		case ChannelType.WindowsStandalone:
		case ChannelType.OSXStandalone:
