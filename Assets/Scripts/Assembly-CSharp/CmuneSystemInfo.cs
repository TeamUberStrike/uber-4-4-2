...
public class CmuneSystemInfo
{
	...
	public CmuneSystemInfo()
	{
		...
		GraphicsPixelFillRate = "Not supported in Unity 5.0+";
		SupportsImageEffects = "Always true";
		SupportsRenderTextures = "Always true";
		SupportsShadows = SystemInfo.supportsShadows.ToString();
		SupportsVertexPrograms = "Required in Unity 5.0+";
		...
		// Application.srcValue is obsolete, extract query string from absoluteURL instead
		string srcValue = "";
		if (Application.platform == RuntimePlatform.WebGLPlayer)
		{
			...
			if (queryIndex >= 0)
			{
				srcValue = url.Substring(queryIndex + 1);
			}
		}
		SrcValue = srcValue;
		...
		MaxAngularVelocity = "Use Rigidbody.maxAngularVelocity instead";
		MinPenetrationForPenalty = "Use Physics.defaultContactOffset instead";
		SleepAngularVelocity = "Use sleepThreshold instead";
		SleepVelocity = "Use sleepThreshold instead";
		SolverIterationCount = Physics.defaultSolverIterations.ToString("N2");
		CurrentResolution = "X " + Screen.width + ", Y " + Screen.height + ", Refresh " + Screen.currentResolution.refreshRateRatio.value.ToString("N0") + "Hz";
