...
public class InboxPageGUI : MonoBehaviour
{
	...
	private void DoReply(Rect rect)
	{
		...
		if (GUI.Button(new Rect(0f, 0f, position.width, position.height), LocalizedStrings.Reply))
		{
			_keyboard = TouchScreenKeyboard.Open(string.Empty, TouchScreenKeyboardType.Default, true, true, false, false);
		}
		GUI.EndGroup();
	}
