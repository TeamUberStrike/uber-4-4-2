...
public class SplineInterpolator : MonoBehaviour
{
	...
	private void Update()
	{
		...
		float num = (mCurrentTime - mNodes[mCurrentIdx].Time) / (mNodes[mCurrentIdx + 1].Time - mNodes[mCurrentIdx].Time);
		float t = MathUtils.Ease(Mathf.Clamp(num, 0f, 0.999999f), mNodes[mCurrentIdx].EaseIO.x, mNodes[mCurrentIdx].EaseIO.y);
		_currentPosition = GetHermiteInternal(mCurrentIdx, num);
		_currentRotation = GetSquad(mCurrentIdx, t);
		if ((bool)mTarget)
		{
			mTarget.transform.position = _currentPosition;
			if (mRotations)
			{
				mTarget.transform.rotation = _currentRotation;
			}
		}
	}
