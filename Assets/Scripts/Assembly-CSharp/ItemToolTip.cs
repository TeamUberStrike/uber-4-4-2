...
public class ItemToolTip : AutoMonoBehaviour<ItemToolTip>
{
	...
	private FloatPropertyBar _defenseBonus = new FloatPropertyBar(LocalizedStrings.DefenseBonus);
	...
	private Action OnDrawItemDetails = delegate
	{
	};

	private Action OnDrawTip = delegate
	{
	};
	...
	private void OnGUI()
	{
		...
		if (IsEnabled && !PanelManager.IsAnyPanelOpen)
		{
			...
			_item.DrawIcon(new Rect(20f, 10f, 48f, 48f), true);
	...
	public void SetItem(IUnityItem item, Rect bounds, PopupViewSide side, int daysLeft = -1, BuyingDurationType duration = BuyingDurationType.None)
	{
		...
		case UberstrikeItemClass.GearHolo:
			...
			_defenseBonus.Value = ((UberStrikeItemGearView)item.View).ArmorAbsorptionPercent;
		...
		case UberstrikeItemClass.WeaponSniperRifle:
		{
			...
			if (uberStrikeItemWeaponView != null)
			{
				...
				_fireRate.Max = WeaponConfigurationHelper.MaxRateOfFire;
				_accuracy.Value = WeaponConfigurationHelper.MaxAccuracySpread - WeaponConfigurationHelper.GetAccuracySpread(uberStrikeItemWeaponView);
		...
		case UberstrikeItemClass.WeaponLauncher:
		{
			...
			if (uberStrikeItemWeaponView2 != null)
			{
				...
				_fireRate.Max = WeaponConfigurationHelper.MaxRateOfFire;
				_velocity.Value = WeaponConfigurationHelper.GetProjectileSpeed(uberStrikeItemWeaponView2);
				_velocity.Max = WeaponConfigurationHelper.MaxProjectileSpeed;
				_damageRadius.Value = WeaponConfigurationHelper.GetSplashRadius(uberStrikeItemWeaponView2);
	...
	{
		ProgressBar(new Rect(20f, 120f, 200f, 12f), _defenseBonus.Title, _defenseBonus.Percent, ColorScheme.ProgressBar, string.Empty);
		ProgressBar(new Rect(20f, 135f, 200f, 12f), _armorCarried.Title, _armorCarried.Percent, ColorScheme.ProgressBar, string.Empty);
	}
