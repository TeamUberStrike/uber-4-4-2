...
public abstract class BaseItemGUI : IShopItemGUI
{
	...
	protected void DrawRecommendPrice(Rect rect, ItemPrice price)
	{
		if (price != null)
		{
			...
			GUI.Label(new Rect(rect.width - vector.x - 127f, rect.y + 1f, 100f, 16f), "FROM", BlueStonez.label_interparkbold_11pt_right);
			GUI.DrawTexture(new Rect(rect.width - vector.x - 20f, rect.y + 1f, 16f, 16f), ShopUtils.CurrencyIcon(price.Currency));
			GUI.Label(new Rect(rect.width - vector.x, rect.y + 1f, vector.x, 16f), text, BlueStonez.label_interparkmed_11pt_right);
		}
	}
