	...
	private void FreezeObject(bool b)
	{
		...
		for (int i = 0; i < _rbs.Length; i++)
		{
			...
			if (b)
			{
				...
				_adrag[i] = rigidbody.angularDamping;
				_drag[i] = rigidbody.linearDamping;
				rigidbody.angularDamping = 10000f;
				rigidbody.linearDamping = 10000f;
				rigidbody.mass = 0.1f;
			}
			else
			{
				rigidbody.mass = _oriMass[i];
				rigidbody.angularDamping = _adrag[i];
				rigidbody.linearDamping = _drag[i];
			}
		}
		_isFreezed = b;
	}
