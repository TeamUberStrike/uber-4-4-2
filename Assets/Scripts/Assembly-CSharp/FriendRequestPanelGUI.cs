...
public class FriendRequestPanelGUI : PanelGuiBase
{
	...
	private void OnGUI()
	{
		if (_keyboard != null)
		{
			...
			else if (_keyboard.text.Length > 140)
			{
				_msgContent = _keyboard.text.Substring(0, 140);
			}
			else
			{
				_msgContent = _keyboard.text;
			}
		}
	...
	private void DoCompose(Rect rect)
	{
		...
		if (GUI.Button(new Rect(num2, num5, num3, 108f), _msgContent, BlueStonez.textArea))
		{
			TouchScreenKeyboard.hideInput = false;
			_keyboard = TouchScreenKeyboard.Open(_msgContent, TouchScreenKeyboardType.Default, false, false, false, false);
