...
public class SpringGrenadeQuickItem : QuickItem, IProjectile, IGrenadeProjectile
{
	...
	private class FlyingState : IState
	{
		...
		public void OnEnter()
		{
			...
			if (!gameObject || !GameState.LocalAvatar.Decorator || gameObjectCollider == null)
			{
				return;
			}
			...
			foreach (CharacterHitArea characterHitArea in hitAreas)
			{
				...
				{
					Collider characterCollider = characterHitArea.GetComponent<Collider>();
					if (characterCollider != null)
					{
						Physics.IgnoreCollision(gameObjectCollider, characterCollider);
					}
				}
			}
		}
		...
		private void OnCollisionEnterEvent(Collision c)
		{
			...
			else if (behaviour._config.IsSticky)
			{
				...
				{
					Collider behaviourCollider = behaviour.GetComponent<Collider>();
					if (behaviourCollider != null)
					{
						behaviour.transform.position = c.contacts[0].point + c.contacts[0].normal * behaviourCollider.bounds.extents.sqrMagnitude;
					}
				}
				behaviour.machine.PopState();
	...
	private class DeployedState : IState
	{
		...
		public void OnEnter()
		{
			...
			Rigidbody rb = behaviour.GetComponent<Rigidbody>();
			if (rb != null)
			{
				rb.isKinematic = true;
			}
			...
			if (behaviourCollider != null)
			{
				UnityEngine.Object.Destroy(behaviourCollider);
			}
			behaviour.gameObject.layer = 2;
			if (behaviour.DeployedEffect != null)
			{
				var emission = behaviour.DeployedEffect.emission;
				emission.enabled = true;
				if (!behaviour.DeployedEffect.isPlaying)
				{
		...
		public void OnTriggerEnterEvent(Collider c)
		{
			...
			{
				SfxManager.Play3dAudioClip(GameAudio.JumpPad, 1f, 0.1f, 10f, AudioRolloffMode.Linear, behaviour.transform.position);
			}
		}
	...
	public IGrenadeProjectile Throw(Vector3 position, Vector3 velocity)
	{
		...
		springGrenadeQuickItem.machine.RegisterState(1, new FlyingState(springGrenadeQuickItem));
		springGrenadeQuickItem.machine.RegisterState(2, new DeployedState(springGrenadeQuickItem));
		springGrenadeQuickItem.machine.PushState(1);
