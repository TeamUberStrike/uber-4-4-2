...
public class ShopPageGUI : PageGUI
{
	...
	private void Start()
	{
		...
		_shopAreaSelection.Add(ShopArea.Inventory, new GUIContent(string.Empty, ShopIcons.LabsInventory, LocalizedStrings.Inventory));
		_shopAreaSelection.Add(ShopArea.Shop, new GUIContent(string.Empty, ShopIcons.LabsShop, LocalizedStrings.Shop));
		_shopAreaSelection.Add(ShopArea.MysteryBox, new GUIContent(string.Empty, ShopIcons.IconLottery, LocalizedStrings.MysteryBox));
		...
		{
			_shopAreaSelection.Add(ShopArea.Credits, new GUIContent(string.Empty, ShopIcons.CreditsIcon32x32, LocalizedStrings.Credits));
		}
	...
	{
		yield return new WaitForEndOfFrame();
		LoadoutAreaChangedEvent loadoutAreaChangedEvent = new LoadoutAreaChangedEvent();
		loadoutAreaChangedEvent.Area = _loadoutAreaSelection.Current;
		CmuneEventHandler.Route(loadoutAreaChangedEvent);
	}
	...
	public override void DrawGUI(Rect rect)
	{
		...
		if (PlayerDataManager.AccessLevel == MemberAccessLevel.Admin)
		{
			...
			if (GUI.Button(new Rect(32f, 100f, 128f, 32f), "Reload Shop", BlueStonez.button_green))
			{
	...
	public void EquipItemFromArea(IUnityItem item, LoadoutSlotType slot, ShopArea area)
	{
		if (item != null && !Singleton<LoadoutManager>.Instance.IsItemEquipped(item.View.ID))
		{
			...
			else if (item.View.LevelLock <= PlayerDataManager.PlayerLevelSecure)
			{
				BuyPanelGUI buyPanelGUI = PanelManager.Instance.OpenPanel(PanelType.BuyItem) as BuyPanelGUI;
				if ((bool)buyPanelGUI)
				{
					buyPanelGUI.SetItem(item, _location, BuyingRecommendationType.None, true);
				}
			}
		}
		else
	...
	public void SelectLoadoutWeapon(LoadoutSlotType slot)
	{
		if (Singleton<InventoryManager>.Instance.CurrentWeaponSlot != slot)
		{
			...
			GameState.LocalAvatar.Decorator.SetActiveWeaponSlot(slot);
			GameState.LocalAvatar.Decorator.ShowWeapon(slot);
			InventoryItem item;
			if (Singleton<LoadoutManager>.Instance.TryGetItemInSlot(slot, out item))
			{
				AutoMonoBehaviour<AvatarAnimationManager>.Instance.SetAnimationState(PageType.Shop, item.Item.View.ItemClass);
			}
			else
			{
	...
	public void UnequipItem(IUnityItem item)
	{
		...
		if (item != null && Singleton<LoadoutManager>.Instance.TryGetSlotForItem(item, out slot))
		{
			...
			case UberstrikeItemType.Weapon:
				AutoMonoBehaviour<AvatarAnimationManager>.Instance.SetAnimationState(PageType.Shop, (UberstrikeItemClass)0);
				GameState.LocalAvatar.Decorator.SetActiveWeaponSlot(slot);
				GameState.LocalAvatar.Decorator.UnassignWeapon(slot);
				break;
			}
	...
	private void ShowUnequipGearFx(IUnityItem item, LoadoutSlotType slot)
	{
		...
		{
			Singleton<TemporaryLoadoutManager>.Instance.ResetGearLoadout(slot);
		...
		AutoMonoBehaviour<AvatarAnimationManager>.Instance.SetAnimationState(PageType.Shop, item.View.ItemClass, slot == LoadoutSlotType.GearHolo);
	}
	...
	public void SelectLoadoutArea(LoadoutArea area)
	{
		...
		LoadoutAreaChangedEvent loadoutAreaChangedEvent = new LoadoutAreaChangedEvent();
		loadoutAreaChangedEvent.Area = area;
		CmuneEventHandler.Route(loadoutAreaChangedEvent);
	}
	...
	private void UpdateShopItems()
	{
		...
		{
			_inventoryItemGUIList.Add(new InventoryItemGUI(allItem, _location));
		}
		...
		{
			_shopItemGUIList.Add(new ShopItemGUI(allShopItem, _location));
		}
	...
	private void DrawShop(Rect labsRect)
	{
		...
		case ShopArea.MysteryBox:
			_lotteryGui.Draw(new Rect(0f, 74f, labsRect.width, labsRect.height - 74f));
			break;
	...
	private void DrawShopTabs(Rect rect)
	{
		...
		int num = UnityGUI.Toolbar(new Rect(1f, 32f, _shopAreaSelection.Length * 44 - 2, 44f), _shopAreaSelection.Index, _shopAreaSelection.GuiContent, BlueStonez.tab_largeicon);
	...
	private void DrawItemGUIList<T>(List<T> list, Rect position) where T : IShopItemGUI
	{
		...
		if (!GuiLockController.IsLocked(GuiDepth.Page))
		{
			list2.Reverse();
			Singleton<ItemLoader>.Instance.SetFirstToLoadItems(list2);
			AutoMonoBehaviour<TextureLoader>.Instance.SetFirstToLoadImages(list2);
		}
	}
	...
	private void DrawQuickItemLoadout(Rect position)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			LoadoutSlotType loadoutSlotType = (LoadoutSlotType)(12 + i);
		...
		if (rect.Contains(Event.current.mousePosition) && !PanelManager.IsAnyPanelOpen && !PopupSystem.IsAnyPopupOpen)
		{
			if (Event.current.type == EventType.MouseDown)
			{
				if (Singleton<InventoryManager>.Instance.CurrentWeaponSlot != slot)
				{
					...
					GameState.LocalAvatar.Decorator.ShowWeapon(slot);
					InventoryItem item2;
					if (Singleton<LoadoutManager>.Instance.TryGetItemInSlot(slot, out item2))
					{
						AutoMonoBehaviour<AvatarAnimationManager>.Instance.SetAnimationState(PageType.Shop, item2.Item.View.ItemClass);
					}
					else
					{
						AutoMonoBehaviour<AvatarAnimationManager>.Instance.SetAnimationState(PageType.Shop, (UberstrikeItemClass)0);
